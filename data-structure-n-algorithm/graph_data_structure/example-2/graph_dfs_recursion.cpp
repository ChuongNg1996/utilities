...
* adj[n] has at least one adj node? Check a visited list/array
    -> YES,
        -> Check for unvisited adj nodes: move to first node of the list (if more than one node).
*/
...
void dft(vector<int> adj[], int V, int i)
{
        ...
        for (auto x : adj[i])
        {
            ...
            {
                // Then go deeper, till all adj nodes are visited.
                dft(adj,V,x);
            }
        }
}
...
int main()
{
    ...
    for (auto x : visitedNode) cout << " " << x;
    cout << "Check the sequence if it fits dfs\n";
    system("pause");
    return 0;
}
